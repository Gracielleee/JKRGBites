...
class PickerFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnReset.setOnClickListener {
            viewModel.resetDeck()
            // After resetting, ensure we force a re-observation or refresh
            viewModel.refreshDeck()
            Toast.makeText(context, "Deck Reset", Toast.LENGTH_SHORT).show()
        }
    }
...
//                    yOffset = 200)
            ...
            SwipeDirection.LEFT -> {
                ...
                targetY = 0f
                targetRotation = -30f
                toastMessage = "Discarded for now."
                toastType = ToastUtils.ToastType.INFO
                ToastUtils.showCustomToast(requireContext(),
                    toastMessage,
                    toastType,
    ...
    private fun observeRestaurantDeck() {
        viewLifecycleOwner.lifecycleScope.launch {
            combine(viewModel.deck, viewModel.allRestaurants) { deck, allRestaurants ->
                ...
                binding.cardCountDesc.visibility = if (deck.isNotEmpty()) View.VISIBLE else View.GONE
